...
class KareeInstaller extends CommandRunner {
    ...
    async install(options = { callback: (status = 0) => { } }) {
        ...
        /**
         * In case of the module creation, this path contains the path of the root application
         */
        ...
        let res = cmd.run(
            ...
            `--android-language ${this.settings.androidSupport} ` +
            ` ${this.pathSeparator}${this.settings.appName} ` +
            ((this.isModule)
        ...
        res
            .on('close', (status, signal) => {
                ...
                if (status == 0) {
                                        ...
                                        callback: () => {
                                            ...
                                            template = (
                                                ...
                                                .replace(
                                                    ...
                                                    `  await KareeModuleLoader.load(${formatter.underscoreToCambel(this.settings.appName)}Module());\n` +
                    ...
                    console.log('\n')
                    console.log('\x1b[31m\x1b[1mError during flutter project creation\x1b[22m\x1b[0m\n');
                    options.callback?.call(1);
                }
            })
            .on('error', (err) => {
