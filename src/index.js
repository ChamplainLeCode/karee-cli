...
class KareeInstaller{
    ...
    async install() {
        ...
        spinner.setSpinnerTitle('Downloading Flutter files')
        spinner.start()
        ...
        res.on('close', (status, signal)=>{
            ...
            if(status == 0){
                ...
                console.log('\n')
                spinner = new Spinner()
                spinner.setSpinnerTitle('Setting up Karee configuration')
                spinner.setSpinnerString('⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏')
                spinner.start()
                ...
                spinner = new Spinner()
                spinner.setSpinnerString('⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏')
                spinner.setSpinnerTitle('Downloading Karee file')
                spinner.start()
                cmd.run('git clone https://github.com/ChamplainLeCode/wp_core_kari.git tmp_karee_conf')
                    .on("close", (code, signalClone) => {
                        ...
                        if(code == 0){
                            ...
                            console.log(cmd.runSync('flutter pub get').data)
                            process.chdir(`lib${path.sep}core${path.sep}screen_tracker`)
                            console.log(cmd.runSync('flutter pub get'))
                            process.chdir('screengen')
                            console.log(cmd.runSync('flutter pub get'))
                            io.delete(`${process.cwd()}${path.sep}build.yaml`)
                            io.writeFile(
                                'targets:\n'+
                                '   $default:\n'+
                                '       builders:\n'+
                            ...
                            console.log(cmd.runSync('flutter pub get'))
                            console.log(cmd.runSync('flutter packages pub run build_runner build --delete-conflicting-outputs'))
